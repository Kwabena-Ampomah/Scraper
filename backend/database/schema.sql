-- Database schema for User Feedback Intelligence Platform

-- Create database (run this manually)
-- CREATE DATABASE feedback_intelligence;
...
CREATE INDEX idx_posts_sentiment ON posts(id) WHERE id IN (SELECT content_id FROM sentiment_analysis WHERE content_type = 'post');
